...
# Embeddings use Google's `gemini-embedding-001` (free tier, same API key as the
# chat model). The Chroma index persists to vectorstore/ so it survives
# restarts and only needs rebuilding when your documents change.
# """
...
# # Cached vector store handle for the running server.
# _vectorstore = None


# def get_vectorstore() -> Chroma:
#     """Open the persisted Chroma index (empty if nothing has been ingested yet)."""
#     global _vectorstore
...
#                 "add files to documents/ and run `python ingest.py`."
#             )
#         blocks = []
